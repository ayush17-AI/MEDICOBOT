...
import { evaluateVitalsEngine, VitalsInput, VitalsEvalResult } from '@/lib/vitalsEngine';
...
export default function VitalsDashboardPage() {
  ...
  useEffect(() => {
    try {
      ...
      console.warn('Failed to load patient from session storage:', e);
    }
  }, []);

  // Run hardware sensor read simulation
  const runSensorRead = (preset: 'normal' | 'mild' | 'anomaly' = 'normal') => {
    ...
    setTimeout(() => {
      ...
      if (preset === 'anomaly') {
        // Extreme Anomaly Preset (110°F temp, 40% SpO2, 35 BPM, 210/130 mmHg)
        data = {
          temperature: 110.0,
          heartRate: 35,
          spo2: 40,
      ...
      setVitals(data);
      const result = evaluateVitalsEngine(data);
  ...
  const handleProceed = async () => {
    ...
    try {
      ...
      console.warn('Supabase record sync notice:', err);
    } finally {
      setIsSyncing(false);
  ...
  const canProceed = Boolean(vitals && evalResult && !evalResult.isAnomaly && !isSyncing);
  ...
  return (
    <div className="min-h-screen bg-slate-50 relative p-4 sm:p-8 flex flex-col items-center overflow-y-auto">
      ...
      <div className="relative z-10 w-full max-w-4xl space-y-6 my-4">
        ...
        <div className="bg-white/90 backdrop-blur-md border border-slate-200 rounded-3xl p-6 shadow-md flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h1 className="text-2xl font-black text-slate-900 tracking-tight mt-0.5">Kiosk Vitals Read & Alert Dashboard</h1>
            </div>
          </div>
        ...
        {/* ANOMALY ERROR ALERT BANNER */}
        {evalResult?.isAnomaly && (
          <div
            data-testid="vitals-anomaly-alert"
            className="p-6 rounded-3xl bg-red-600 text-white shadow-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 animate-shake border-2 border-red-700"
          >
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-red-100 mt-1 font-medium">
                  Readings cross human physiological bounds. Please re-attach hardware sensors and re-read.
                </p>
              </div>
            </div>
            ...
            >
              ...
              <span>Re-Read Hardware Sensors</span>
            </button>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          >
            ...
            <div className="mt-2">
              {evalResult ? (
                ...
                >
                  {evalResult.temperature.status === 'ANOMALY_ERROR' && <AlertTriangle size={12} />}
                  {evalResult.temperature.status === 'NORMAL' && <CheckCircle2 size={12} />}
                  {evalResult.temperature.message}
                </span>
              ) : (
                <span className="text-xs font-medium text-slate-400">Click &quot;Read Sensors&quot;</span>
              )}
            </div>
          </div>
          ...
          >
            ...
            <div className="mt-2">
              {evalResult ? (
                ...
                >
                  {evalResult.heartRate.status === 'ANOMALY_ERROR' && <AlertTriangle size={12} />}
                  {evalResult.heartRate.status === 'NORMAL' && <CheckCircle2 size={12} />}
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-bold uppercase tracking-wider text-slate-500">SpO2 Oxygen</span>
            ...
            <div className="mt-2">
              {evalResult ? (
                ...
                >
                  {evalResult.spo2.status === 'ANOMALY_ERROR' && <AlertTriangle size={12} />}
                  {evalResult.spo2.status === 'NORMAL' && <CheckCircle2 size={12} />}
                  {evalResult.spo2.message}
                </span>
              ) : (
                <span className="text-xs font-medium text-slate-400">Click &quot;Read Sensors&quot;</span>
              )}
          ...
          >
            ...
            <div className="mt-2">
              {evalResult ? (
                ...
                >
                  {evalResult.bp.status === 'ANOMALY_ERROR' && <AlertTriangle size={12} />}
                  {evalResult.bp.status === 'NORMAL' && <CheckCircle2 size={12} />}
                  {evalResult.bp.message}
                </span>
              ) : (
                <span className="text-xs font-medium text-slate-400">Click &quot;Read Sensors&quot;</span>
              )}
        ...
        <div className="bg-white/90 backdrop-blur-md border border-slate-200 rounded-3xl p-6 shadow-md flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          >
            <RefreshCw size={18} className={isReading ? 'animate-spin' : ''} />
            <span>{isReading ? 'Reading Hardware Sensors...' : 'Read Hardware Sensors'}</span>
          </button>
          ...
          >
            <span>{isSyncing ? 'Syncing Record...' : 'Proceed to Symptom Analysis'}</span>
            <ArrowRight size={18} />
          </button>
        </div>
      </div>
